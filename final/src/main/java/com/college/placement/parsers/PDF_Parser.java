...
public class PDF_Parser {
	...
	public static void parseAvis(String toParse) {
		...
		for (int i = 0; i < parsed.length; i++) {
			...
			} else if (parsed[i].contains("Due") && parsed[i - 1].contains("Amount")) {// for net amount and currency
																						// type
				data.add(parsed[i + 1]);
				data.add(parsed[i + 2]);
			...
			} else if (parsed[i].contains("location") && parsed[i - 1].contains("Return")) {// for Destination and
																							// Travel Date
				data.add(parsed[i + 2]);
			...
			} else if (parsed[i].contains("by") && parsed[i - 1].contains("Rented")) {// for
	...
	public static void parseEnterprise(String toParse) throws IOException {
		...
		for (String c : concatted) {
			...
			for (int i = 0; i < parsed.length; i++) {
				...
				} else if (parsed[i].contains("Location:")) {// for destination (1-3 words)
					if (parsed[i + 4].contains("Reserved")) {
						data.add(parsed[i + 1] + " " + parsed[i + 2] + " " + parsed[i + 3]);
					} else if (parsed[i + 3].contains("Reserved")) {
						data.add(parsed[i + 1] + " " + parsed[i + 2]);
					} else if (parsed[i + 2].contains("Reserved")) {
						data.add(parsed[i + 1]);
					}
				} else if (parsed[i].contains("VAT") && parsed[i - 1].contains("&")) {// for currency type and
																						// commission amount
					data.add(parsed[i + 2]);
					data.add(parsed[i + 1].replaceAll("\\(|\\)", ""));
				} else if (parsed[i].contains("Paid") && parsed[i - 1].contains("be")) {
					data.add(parsed[i + 1]);
